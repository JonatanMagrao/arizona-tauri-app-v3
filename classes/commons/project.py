        ...
        try:
            ...
            if monday_url.match(link):
                ...
                if len(monday_links) == 0:
                    projetos.append(
                        {"msg": f"No gdrive links found on pinned updates on: {link}"})
                    continue
...
def copy_projects(projetos: list[SuperplayProject]):
    ...
    for projeto in projetos:
        if isinstance(projeto, dict) and projeto["msg"]:
            continue
        try:
...
def project_metadata(projetos: list):
    manifest_list = []
    for projeto in projetos:
        if isinstance(projeto, dict) and projeto["msg"]:
            manifest_list.append({"msg": projeto["msg"]})
            continue
